...
// =============== MODULE: Preload Video (1 lần/tab) ===============
const PreloadVideo = {
  init() {
    ...
    // Nếu đã xem preload trong tab này => ẩn luôn
    if (sessionStorage.getItem('preloadPlayed')) {
      if (preload) preload.style.display = 'none';
      if (main) main.style.display = 'block';
      return;
    }

    // Chưa xem => chạy preload
    this.handleClickToSkip();
    this.handleEndedAutoHide();

    // Đánh dấu đã xem khi video bắt đầu chạy
    if (video) {
      video.addEventListener('play', () => {
        sessionStorage.setItem('preloadPlayed', 'true');
      });
    }
  },
  ...
  handleClickToSkip() {
    document.addEventListener('click', function skipPreload() {
      ...
      if (preload && !preload.classList.contains('hidden')) {
        ...
        sessionStorage.setItem('preloadPlayed', 'true'); // đánh dấu đã xem
